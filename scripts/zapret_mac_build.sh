...
log "${Y}=== Zapret для macOS (сборка из исходников) ===${X}"
...
# Создаем Makefile для macOS
log "${Y}Создаю Makefile для macOS...${X}"
cat > Makefile.macos << 'MAKEFILE'
CC = gcc
CFLAGS = -std=gnu99 -Os -flto=auto -D_GNU_SOURCE -D_DARWIN_C_SOURCE
LDFLAGS = -lz -lpthread

OBJS = tpws.c tpws_conn.c helpers.c hostlist.c ipset.c params.c pools.c protocol.c redirect.c resolver.c sec.c tamper.c gzip.c
